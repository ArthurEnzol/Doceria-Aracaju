...
export function Cardapio({ onAddToCart, onNavigate }: CardapioProps) {
  ...
  // Buscar doces e categorias da API
  useEffect(() => {
    fetchData();
  }, []);
  ...
  async function fetchData() {
    try {
      const [docesRes, categoriasRes] = await Promise.all([
        fetch(`${API_URL}/doces`),
        fetch(`${API_URL}/categorias`)
      ]);
      
      if (docesRes.ok) {
        const docesData = await docesRes.json();
        const mappedData = docesData.map((d: any) => ({
          ...d,
          id: d._id || d.id
        }));
        setDoces(mappedData);
  ...
  // Criar array de filtros com 'all' + nomes das categorias
  const nomesCategorias = ['all', ...categorias.map(c => c.nome)];

  const filteredDoces = filter === 'all' 
    ? doces 
    : doces.filter(d => d.categoria === filter);
  ...
  if (loading) {
    return (
      <section ref={sectionRef} id="cardapio" className="relative py-24 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-[#2D1B2E]/70">Carregando cardápio...</p>
        </div>
      </section>
    );
  }
